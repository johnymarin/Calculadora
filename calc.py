...
def get_terms(string, position):
    ...
    if previous_operation:
        ...
        # -- If previous operator is minus symbol (-) we assume is the sign of the number and we add it --
...
def run(string):
    ...
    # -- Until we don't fine more oerators in the string we keep looping --
    result = 0
    while still_operators(string):

        operation = next_operator(string)
        position = string.index(operation)
        # -- Avoid minus symbol (-) ath the beginning --
        ...
        # -- if the second one has preference  over the first one.  Otherwise we just calculate --
